...
flowchart LR
  ...
  partner_access["Доступы контрагентов"] --> source
  ...
  economics --> decision["Решение: участвовать / отказ / ручной режим"]
  decision --> submission["Подача документов"]
  submission --> contract["Договор, счет, закупка"]
  contract --> execution["Исполнение и закрытие"]
  execution --> final["Финальный расчет"]
```
...
| 12 | Договор, спецификация, счет | документы победы/договора | later |
| 13 | Счет, оплата | план/факт оплат | later |
| 14 | Закупка у поставщика | supplier order/procurement task | later |
| 15 | Исполнение | контроль исполнения контракта | later |
| 16 | Закрывающие документы | акты, УПД, счета-фактуры | later |
| 17 | Репутационные документы | отзывы/подтверждения/архив | later |
| 18 | Финальный расчет | итоговая маржа, факт затрат, уроки | later |
...
## Доступы контрагентов к источникам

### Новый смысл

Для ускорения наполнения базы ASTS может запрашивать у контрагентов/клиентов,
в том числе из HYG-контекста, их разрешенные ключи, токены или учетные доступы
к первичным источникам закупок. С помощью этих доступов система самостоятельно
собирает процедуры, документы, карточки заказчиков и историю обновлений, не
ожидая ручных Excel-выгрузок.

Это не должно превращаться в неуправляемое хранение чужих паролей. Нужны
письменное разрешение, понятные границы доступа и безопасное хранение секретов.
...
Для каждого контрагента храним:

- кто выдал доступ;
- к какому источнику доступ: ЕИС, ЭТП, ФНС, CRM, архив выгрузок;
- тип доступа: API key, token, certificate, OAuth, login/password, file export;
- разрешенный scope: чтение карточек, чтение документов, реестры, история;
- срок действия доступа;
- дату последней проверки;
- ответственного за доступ;
- статус: `requested`, `active`, `expired`, `revoked`, `failed`;
- ссылку на согласие/договор/письмо, если требуется;
- журнал использования.

Секреты не храним в обычных таблицах и не коммитим в репозиторий. В базе
должны быть только ссылки на secret storage и технические метаданные.

### Что запрашивать у контрагента

- список источников, которыми он реально пользуется;
- официальные API-ключи или токены, если они есть;
- доступы к личным кабинетам только если это разрешено правилами площадки;
- сертификаты/ключи только через безопасный канал и с фиксацией владельца;
- примеры закупок, которые должны находиться через этот доступ;
- правила использования доступа и ограничения по частоте запросов;
- контакт ответственного, кто может продлить или отозвать доступ.

### Не потерять

- доступ должен использоваться только для чтения, если иное явно не согласовано;
- нельзя смешивать данные разных контрагентов без `organization_id` и источника;
- каждый импорт должен сохранять raw payload, hash и дату получения;
- при отзыве доступа новые синхронизации останавливаются, но уже полученные
  raw-данные сохраняются по условиям договора;
- ключи надо ротировать и уметь быстро отключать;
- каждый запуск синхронизации должен попадать в audit log.
...
|---|---|---|---|---|
| Воронка сделки | yes | yes | yes | partial |
