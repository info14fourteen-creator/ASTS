...
const expectedCommand = "npm run smoke:eis-real-network-approval-live-route-readme-workflow-copy";
...
const expectedPlanMarker = 'data-testid="eis-real-network-approval-live-route-readme-workflow-copy"';
const expectedReadmeRenderedCommand = "npm run smoke:eis-real-network-approval-live-route-readme-rendered-copy";
...
const expectedSourceMarker = 'data-testid="eis-real-network-approval-live-route-readme-rendered-copy"';
...
assert(
  packageJson.scripts?.["smoke:eis-real-network-approval-live-route-readme-workflow-copy"] ===
    "node scripts/eis-real-network-approval-live-route-readme-workflow-copy.mjs",
  "package.json must expose EIS README workflow copy smoke",
);

assert(workflow.includes(`name: ${expectedWorkflowName}`), "workflow must keep Web build name");
assert(
  workflow.includes(`run: ${expectedReadmeRenderedCommand}`),
  "Web build must keep EIS README rendered copy smoke",
);
assert(workflow.includes(`run: ${expectedCommand}`), "Web build must run EIS README workflow copy");
...
assert(
  ...
  "EIS README workflow copy smoke must run after README rendered copy",
);
assert(
  workflow.indexOf(`run: ${expectedCommand}`) < workflow.indexOf(`run: ${expectedWorkflowFailureCommand}`),
  "EIS README workflow copy smoke must run before workflow failure copy",
);
assert(
  workflow.indexOf(`run: ${expectedCommand}`) < workflow.indexOf(`run: ${expectedNextDomainCommand}`),
  "EIS README workflow copy smoke must run before AI review checks",
);
assert(workflowFileSmoke.includes(expectedCommand), "workflow file smoke must count EIS README workflow command");
assert(workflowFileSmoke.includes(expectedPlanMarker), "workflow file smoke must require EIS README workflow marker");

assert(planPage.includes(expectedSourceMarker), "/plan must keep EIS README rendered marker");
assert(planPage.includes(expectedPlanMarker), "/plan must expose EIS README workflow marker");
assert(
  planPage.includes("eisRealNetworkApprovalLiveRouteReadmeWorkflowCopy"),
  "/plan must expose EIS README workflow data",
);
assert(planPage.includes(expectedApiRoute), "/plan must expose EIS connectors API route");
assert(planPage.includes(expectedCommand), "/plan must expose EIS README workflow command");
...
assert(planPage.includes(expectedReadmeRenderedCommand), "/plan must expose EIS README rendered command");
assert(planPage.includes(expectedWorkflowFailureCommand), "/plan must expose workflow failure command");
assert(planPage.includes(expectedRepairTargets), "/plan must expose EIS README workflow repair targets");
assert(planPage.includes("Data owner + CI owner + QA owner"), "/plan must expose EIS README workflow owners");
assert(planPage.includes(expectedNoMergeCopy), "/plan must show EIS README workflow no-merge copy");

assert(
  routeSmoke.includes('data-testid=\\"eis-real-network-approval-live-route-readme-workflow-copy\\"'),
  "route smoke must require EIS README workflow marker",
);
assert(routeSmoke.includes(`data-command=\\"${expectedCommand}\\"`), "route smoke must require EIS README workflow command");
assert(
...
assert(
  ...
  "route smoke must require EIS README rendered command",
);
assert(
...
assert(
  ...
  "route smoke must require EIS README workflow no-merge copy",
);
assert(
...
console.log(`PASS EIS real-network approval live route README workflow copy (${expectedReadmeRenderedCommand})`);
