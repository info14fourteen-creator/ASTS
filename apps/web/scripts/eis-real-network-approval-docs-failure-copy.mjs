...
const expectedCommand = "npm run smoke:eis-real-network-approval-docs-failure-copy";
...
const expectedPlanMarker = 'data-testid="eis-real-network-approval-docs-failure-copy"';
...
assert(apiReadme.includes("safe EIS procedure"), "API README must keep safe EIS procedure no-merge copy");
...
assert(planPage.includes(expectedPlanMarker), "/plan must expose EIS approval docs failure copy marker");
assert(planPage.includes(expectedCommand), "/plan must expose EIS approval docs failure command");
assert(planPage.includes(expectedDocsHref), "/plan must expose EIS approval docs href");
assert(planPage.includes(expectedFailingCommand), "/plan must expose failing EIS approval command");
assert(planPage.includes(expectedNoMergeCopy), "/plan must expose EIS approval docs no-merge copy");
assert(planPage.includes("Data owner + Docs owner + QA owner"), "/plan must expose EIS approval docs owners");
assert(planPage.includes("data-failing-command={eisRealNetworkApprovalDocsFailureCopy.failingCommand}"), "/plan must expose docs failing command data attr");
assert(planPage.includes("data-parity-command={eisRealNetworkApprovalDocsFailureCopy.parityCommand}"), "/plan must expose docs parity command data attr");

assert(routeSmoke.includes('data-testid=\\"eis-real-network-approval-docs-failure-copy\\"'), "route smoke must require EIS approval docs failure copy marker");
assert(routeSmoke.includes(`data-command=\\"${expectedCommand}\\"`), "route smoke must require EIS approval docs failure command");
...
assert(routeSmoke.includes(`data-no-merge-copy=\\"${expectedNoMergeCopy}\\"`), "route smoke must require EIS approval docs no-merge copy");
assert(routeSmoke.includes("Что делать, если EIS approval docs anchor упал"), "route smoke must require visible EIS approval docs heading");

assert(workflow.includes(`name: ${expectedWorkflowName}`), "workflow must keep Web build name");
assert(workflow.includes(`run: ${expectedCommand}`), "Web build must run EIS approval docs failure copy smoke");
assert(
  workflow.indexOf("run: npm run smoke:eis-real-network-approval-smoke-failure-copy") <
    workflow.indexOf(`run: ${expectedCommand}`),
  "EIS approval docs failure copy smoke must run after EIS approval smoke failure copy",
);
assert(
  workflow.indexOf(`run: ${expectedCommand}`) < workflow.indexOf("run: npm run smoke:ai-review-actions"),
  "EIS approval docs failure copy smoke must run before AI review action parity",
);

if (failures.length > 0) {
  console.error("FAIL EIS real-network approval docs failure copy");
...
console.log(`PASS EIS real-network approval docs failure copy (${expectedRouteCount} routes, ${expectedApprovalCount} approvals)`);
