...
const expectedCommand = "npm run smoke:eis-real-network-approval-live-route-rendered-copy";
const expectedFailingCommand = "npm run smoke -- --url http://127.0.0.1:4177/";
const expectedLiveFailureCommand = "npm run smoke:eis-real-network-approval-live-route-failure-copy";
...
const expectedPlanMarker = 'data-testid="eis-real-network-approval-live-route-rendered-copy"';
...
const expectedSourceMarker = 'data-testid="eis-real-network-approval-live-route-failure-copy"';
...
assert(
  packageJson.scripts?.["smoke:eis-real-network-approval-live-route-rendered-copy"] ===
    "node scripts/eis-real-network-approval-live-route-rendered-copy.mjs",
  "package.json must expose EIS approval live route rendered copy smoke",
);

assert(workflow.includes(`name: ${expectedWorkflowName}`), "workflow must keep Web build name");
assert(workflow.includes(`run: ${expectedLiveFailureCommand}`), "Web build must keep EIS approval live route failure copy smoke");
assert(workflow.includes(`run: ${expectedCommand}`), "Web build must run EIS approval live route rendered copy smoke");
...
assert(
  ...
  "EIS approval live route rendered copy smoke must run after live route failure copy smoke",
);
assert(
  workflow.indexOf(`run: ${expectedCommand}`) < workflow.indexOf(`run: ${expectedWorkflowFailureCommand}`),
  "EIS approval live route rendered copy smoke must run before workflow failure copy smoke",
);
assert(
  workflow.indexOf(`run: ${expectedCommand}`) < workflow.indexOf("run: npm run smoke:ai-review-actions"),
  "EIS approval live route rendered copy smoke must run before AI review checks",
);

assert(planPage.includes(expectedSourceMarker), "/plan must keep EIS approval live route failure marker");
assert(planPage.includes(expectedPlanMarker), "/plan must expose EIS approval live route rendered copy marker");
assert(planPage.includes("eisRealNetworkApprovalLiveRouteRenderedCopy"), "/plan must expose EIS approval live route rendered copy data");
assert(planPage.includes(expectedCommand), "/plan must expose EIS approval live route rendered command");
...
assert(planPage.includes(expectedRepairTargets), "/plan must expose EIS approval live route rendered repair targets");
assert(planPage.includes("Data owner + CI owner + QA owner"), "/plan must expose EIS approval live route rendered owners");
...
assert(routeSmoke.includes('data-testid=\\"eis-real-network-approval-live-route-rendered-copy\\"'), "route smoke must require EIS approval live route rendered marker");
assert(routeSmoke.includes(`data-command=\\"${expectedCommand}\\"`), "route smoke must require EIS approval live route rendered command");
...
console.log(`PASS EIS real-network approval live route rendered copy (${expectedLiveFailureCommand})`);
